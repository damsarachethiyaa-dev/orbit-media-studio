// Security helpers for server/index.mjs. `startEgressProxy` runs a local
// forward proxy that yt-dlp/ffmpeg are pointed at, so every outbound
// connection they make gets DNS-checked and blocked if it resolves to a
// private/internal address (SSRF protection) rather than a real public host.
...
export function validateUrl(value) {
  ...
  if (
    ...
    throw new Error(
      'Use a public http or https link without credentials or custom ports.',
    );
...
export async function startEgressProxy() {
  const server = http.createServer(async (req, res) => {
    try {
      ...
      outgoing.on('error', () => {
        ...
        res.end('Source connection failed.');
      });
  ...
  server.on('connect', async (req, client, head) => {
    try {
      ...
      const upstream = net.connect({ host: ip.address, port: 443 }, () => {
