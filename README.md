...
Engine files are stored in `.media/`, outside source control. Completed downloads persist across restarts. Delete a history entry to remove its exported file. Active jobs become failed after an engine restart. Uploads expire after 24 hours while the engine runs; uploaded files left by an earlier engine instance are cleaned on startup. Finished files are sent to the browser's downloads automatically; turn that off with "Save to device automatically" in Preferences. Only jobs that finish while the page is open save themselves, so reopening the app never re-downloads an existing library. Tapping the save icon prepares the file, then offers Share file and Download file where native sharing is available. Share file uses a fresh tap to open the device's share sheet; available apps and gallery targets depend on the device. Browsers cannot write directly to the photo gallery. Preferences are stored only on this browser, and API keys only for this browser session.
...
| `ORBIT_COOKIES_FILE`    | Optional path to a Netscape-format `cookies.txt` (e.g. exported from your browser) used for sources that require a logged-in session, on supported platforms. Keep this file private; it is never read or transmitted by anything other than yt-dlp on your own machine. |
...
## Source sessions on the hosted engine

The engine already uses the current stable yt-dlp and a YouTube PO token provider. Tokens do not guarantee that a platform accepts the VPS IP. Login-required content needs a valid session with access to the video; no DRM or account-access bypass is implemented.

For your own authorized sessions, place Netscape-format cookie exports on the VPS in `/root/orbit/.secrets/cookies/`: `youtube.txt`, `vimeo.txt`, `reddit.txt`, `facebook.txt`, `instagram.txt`, `tiktok.txt` or `twitter.txt`. Keep them out of chat, source control and the browser bundle. Make them readable only by the engine user (UID 1000 in this image). Compose mounts this directory read-only; each extraction uses a private temporary copy and deletes it afterward. Updates to the mounted files apply to new requests without restarting the engine. Refresh expired sessions when needed.

Outside Compose, set `ORBIT_COOKIES_DIR` to that directory. `ORBIT_COOKIES_FILE` remains an optional fallback jar; a missing configured fallback gives an explicit error. Set `ORBIT_POT_PROVIDER_URL` only when a token service is running; Compose supplies the internal URL automatically. If a valid session still receives an IP-block response, the source is refusing that server network and cannot be made universally available by a frontend change.
